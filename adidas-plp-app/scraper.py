...
def market_for_url(url):
    return 'CA' if urlsplit(url).netloc == 'www.adidas.ca' else 'US'
...
def normalize_url(value):
    ...
    if p.scheme != 'https' or 'https://' + p.netloc != config['origin'] or not re.fullmatch('/' + config['path'] + r'/[a-zA-Z0-9_-]+/?', p.path):
        raise InputError('Use an Adidas US /us/ or Adidas Canada /en/ category URL.')
...
def canada_product(item, filters):
    ...
    link = item.get('url', '')
    origin = MARKETS['CA']['origin']
    relative = link[len(origin):] if isinstance(link, str) and link.startswith(origin + '/en/') else ''
    ...
    return product({
        ...
        'sport': selected.get('sport_en_ca', ''),
...
def fetch_page(url, start=0, market=None):
    ...
    try:
        ...
        if detected == 'CA':
            ...
            endpoint = f"{config['origin']}/plp-app/_next/data/{CA_BUILD_ID}/en/{taxonomy}.json"
            params = {'start': start, 'path': 'en', 'taxonomy': taxonomy}
        else:
    ...
    if detected == 'CA' and r.status_code == 404:
    ...
    try:
        if detected == 'CA':
            ...
            if raw.get('sitePath') != 'en' or raw.get('taxonomy') != taxonomy or raw.get('context', {}).get('locale') != 'en_CA':
                raise AdidasError('Canada returned a different market or category.')
            listing = raw['info']
            rows = [canada_product(i, raw.get('selectedFilters', [])) for i in raw['products']]
        else:
            raw = r.json()['raw']
            listing = raw['itemList']
            rows = [product(i, detected) for i in listing['items']]
    ...
    return dict(filters=canada_filters(raw) if detected == 'CA' else us_filters(raw), url=url, market=detected, currency=config['currency'], title=raw.get('title') or 'Products', total=total, page_size=size,
